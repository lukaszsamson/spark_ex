...
Credo runs with `.credo.exs` defaults — no flags, no disabled checks. Do not silence checks to make the gate pass; fix the code instead.
...
Do not claim a task done if any gate fails.
