defmodule SparkEx.M11.ColumnTest do
  ...
  describe "when_/3 chaining" do
    test "appends additional when branch to existing chain" do
      base =
        Functions.when_(
          Column.gt(Functions.col("salary"), Functions.lit(65_000)),
          Functions.lit("senior")
        )

      result =
        base
        |> Column.when_(Column.gt(Functions.col("salary"), Functions.lit(55_000)), "mid")
      ...
      assert %Column{
               expr:
                 {:fn, "when",
                  [
                    {:fn, ">", [{:col, "salary"}, {:lit, 65_000}], false},
                    {:lit, "senior"},
                    {:fn, ">", [{:col, "salary"}, {:lit, 55_000}], false},
                    {:lit, "mid"},
                    {:lit, "junior"}
                  ], false}
             } = result
