defmodule SparkEx.Unit.ReattachTest do
  ...
  describe "reattach behavior with callback hooks" do
    ...
    test "operation-not-found during reattach re-executes from scratch and discards partial buffer",
         %{session: session} do
      # Per A6 (BUGS_PLAN.txt): we recover from OPERATION/SESSION_NOT_FOUND
      # even when items have arrived. Buffered responses are dropped (they
      # would be regenerated by the fresh ExecutePlan, causing duplicates).
      parent = self()
