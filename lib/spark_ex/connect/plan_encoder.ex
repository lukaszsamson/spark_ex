defmodule SparkEx.Connect.PlanEncoder do
  ...
  def encode(plan, counter) do
    ...
    case plan do
      ...
      _ ->
        ...
        relation = renumber_plan_ids(relation, counter)
        {%Plan{op_type: {:root, relation}}, counter}
    end
  end

  # Stable plan_ids assigned at DataFrame construction reflect the order
  # DataFrames are *built* (sql first, transpose second → sql=0,
  # transpose=1). When that tree is encoded, the carrier wrapper overrides
  # the encoder's per-pass counter ids, so the encoded `RelationCommon.plan_id`
  # values come out construction-order rather than outer-first depth-first.
  #
  # Spark Connect 3.5.x's analyzer tolerates root relations with
  # unrecognized rel_types (e.g. `Transpose`, `UnresolvedTableValuedFunction`)
  # only when the root's `plan_id == 0`; non-zero hits a strict path that
  # raises `IndexOutOfBoundsException`. PySpark naturally produces root=0
  # because its allocator increments per relation in encode order; SparkEx
  # produces root=N (a high construction-order id) before normalization.
  #
  # Walk the encoded relation tree depth-first (outer first), assign
  # sequential ids starting at 0, and apply the same renumber to any
  # `UnresolvedAttribute / UnresolvedRegex / UnresolvedStar / SubqueryExpression
  # plan_id` so column references continue to point at the relation that
  # produces them. Effect: encoded wire bytes match pre-refactor numerically;
  # cross-DataFrame stable id allocation is invisible at the wire level.
  defp renumber_plan_ids(relation, _counter) do
    {_, {_next, mapping}} = collect_plan_id_mapping(relation, {0, %{}})
    apply_plan_id_mapping(relation, mapping)
  end

  defp collect_plan_id_mapping(%Relation{common: common} = relation, acc) do
    acc = record_plan_id(common && common.plan_id, acc)
    {_, acc} = collect_plan_id_mapping(relation.rel_type, acc)
    {relation, acc}
  end

  defp collect_plan_id_mapping(%Expression.UnresolvedAttribute{plan_id: id} = node, acc) do
    {node, record_plan_id(id, acc)}
  end

  defp collect_plan_id_mapping(%Expression.UnresolvedRegex{plan_id: id} = node, acc) do
    {node, record_plan_id(id, acc)}
  end

  defp collect_plan_id_mapping(%Expression.UnresolvedStar{plan_id: id} = node, acc) do
    acc = record_plan_id(id, acc)
    descend_collect(node, acc)
  end

  defp collect_plan_id_mapping(%Spark.Connect.SubqueryExpression{plan_id: id} = node, acc) do
    acc = record_plan_id(id, acc)
    descend_collect(node, acc)
  end

  defp collect_plan_id_mapping(%_{} = struct, acc), do: descend_collect(struct, acc)

  defp collect_plan_id_mapping(list, acc) when is_list(list),
    do:
      {list,
       Enum.reduce(list, acc, fn item, a ->
         {_, a} = collect_plan_id_mapping(item, a)
         a
       end)}

  defp collect_plan_id_mapping(tuple, acc) when is_tuple(tuple) do
    acc =
      tuple
      |> Tuple.to_list()
      |> Enum.reduce(acc, fn item, a ->
        {_, a} = collect_plan_id_mapping(item, a)
        a
      end)
  ...
  defp collect_plan_id_mapping(map, acc) when is_map(map) do
    acc =
      map
      |> Map.values()
      |> Enum.reduce(acc, fn item, a ->
  ...
  defp collect_plan_id_mapping(other, acc), do: {other, acc}

  defp record_plan_id(id, {next, mapping} = acc) when is_integer(id) do
    if Map.has_key?(mapping, id) do
      acc
    else
      {next + 1, Map.put(mapping, id, next)}
    end
  end

  defp record_plan_id(_, acc), do: acc
  ...
  defp apply_plan_id_mapping(%Relation{common: common} = relation, mapping) do
    new_common = remap_common(common, mapping)
    new_rel_type = apply_plan_id_mapping(relation.rel_type, mapping)
    %{relation | common: new_common, rel_type: new_rel_type}
  end

  defp apply_plan_id_mapping(%Expression.UnresolvedAttribute{} = node, mapping) do
    %{node | plan_id: Map.get(mapping, node.plan_id, node.plan_id)}
  end

  defp apply_plan_id_mapping(%Expression.UnresolvedRegex{} = node, mapping) do
    %{node | plan_id: Map.get(mapping, node.plan_id, node.plan_id)}
  end

  defp apply_plan_id_mapping(%Expression.UnresolvedStar{} = node, mapping) do
    rewritten = %{node | plan_id: Map.get(mapping, node.plan_id, node.plan_id)}
    apply_to_struct_fields(rewritten, mapping)
  end

  defp apply_plan_id_mapping(%Spark.Connect.SubqueryExpression{} = node, mapping) do
    rewritten = %{node | plan_id: Map.get(mapping, node.plan_id, node.plan_id)}
    apply_to_struct_fields(rewritten, mapping)
  end

  defp apply_plan_id_mapping(%_{} = struct, mapping), do: apply_to_struct_fields(struct, mapping)

  defp apply_plan_id_mapping(list, mapping) when is_list(list),
    do: Enum.map(list, &apply_plan_id_mapping(&1, mapping))

  defp apply_plan_id_mapping(tuple, mapping) when is_tuple(tuple) do
    tuple |> Tuple.to_list() |> Enum.map(&apply_plan_id_mapping(&1, mapping)) |> List.to_tuple()
  end

  defp apply_plan_id_mapping(map, mapping) when is_map(map),
    do: Map.new(map, fn {k, v} -> {k, apply_plan_id_mapping(v, mapping)} end)

  defp apply_plan_id_mapping(other, _mapping), do: other

  defp remap_common(nil, _mapping), do: nil

  defp remap_common(%RelationCommon{plan_id: nil} = common, _mapping), do: common

  defp remap_common(%RelationCommon{plan_id: id} = common, mapping) do
    %{common | plan_id: Map.get(mapping, id, id)}
  end

  defp apply_to_struct_fields(struct, mapping) do
    fields =
      struct
      |> Map.from_struct()
      |> Enum.map(fn {k, v} -> {k, apply_plan_id_mapping(v, mapping)} end)

    Kernel.struct(struct.__struct__, fields)
  end
