defmodule SparkEx.Column do
  ...
  def cast(%__MODULE__{} = col, %Spark.Connect.DataType{} = type) do
    %__MODULE__{expr: {:cast, col.expr, type}}
  end
  ...
  def when_(%__MODULE__{expr: {:fn, "when", args, false}}, %__MODULE__{} = condition, %__MODULE__{} = value) do
    ...
    %__MODULE__{expr: {:fn, "when", args ++ [condition.expr, value.expr], false}}
  end

  def when_(%__MODULE__{expr: {:fn, "when", args, false}}, %__MODULE__{} = condition, value) do
    if rem(length(args), 2) == 1 do
      raise ArgumentError, "otherwise() has already been called on this when() expression"
    end

    %__MODULE__{expr: {:fn, "when", args ++ [condition.expr, {:lit, value}], false}}
  end

  def when_(%__MODULE__{}, _condition, _value) do
    raise ArgumentError,
          "when/3 can only be applied on a Column previously generated by when/2"
  end
  ...
  def astype(%__MODULE__{} = col, %Spark.Connect.DataType{} = type) do
    cast(col, type)
  end
